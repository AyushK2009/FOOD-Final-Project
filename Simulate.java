...
public class Simulate {
    ...
    // This following code is a temporary graph I've made, it's very rough and should be replaced in the final copy.
    
    public void printConsoleGraph() {
    System.out.println("\n--- Flight Graph (Y: Position, X: Time) ---");
    
    int rows = 15; // Height of the graph
    int cols = 60; // Width of the graph
    ...
    double tripDist = states.getTripDistance();

    // Map data points into our text grid dimensions
    for (int i = 0; i < times.size(); i++) {
        double t = times.get(i);
        // Calculate distance *away* from destination
        double distAway = tripDist - positions.get(i);

        // Normalize scaling to fit our rows and columns
        int xNormalized = (int) ((t / maxTime) * (cols - 1));
        // Invert Y because row 0 is the top of the terminal screen
        int yNormalized = (int) ((distAway / tripDist) * (rows - 1));

        // Ensure bounds safety
        xNormalized = Math.max(0, Math.min(cols - 1, xNormalized));
        yNormalized = Math.max(0, Math.min(rows - 1, yNormalized));

        grid[yNormalized][xNormalized] = '*'; // Plot point
    }

    // Render the grid to the console with a Y-axis line
    for (int r = 0; r < rows; r++) {
        System.out.print("|"); // Y-axis boundary
        for (int c = 0; c < cols; c++) {
    ...
    System.out.println("0" + " ".repeat(cols - 2) + "Time ->");
}
}
